...
def main(cfg):
    ...
    while num_samples < total_samples:
        ...
        if len(noise) >= audio_length: # Trim noise if the noise is longer than the audio length
        ...
        if len(clean) > audio_length: # Trim clean speech if the speech is longer than the noise length
            ...
            while len(clean) < audio_length:
                ...
                silence_length = int(fs * np.random.uniform(silence_lower, silence_upper))
                start = labels[-1][1] + silence_length # Start time of next audio clip 
